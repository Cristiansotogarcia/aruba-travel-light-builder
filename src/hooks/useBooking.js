...
const useBooking = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            if (error) {
                ...
                const productsWithStatus = data.map(item => {
                    ...
                    let availability_status; // Renamed status to availability_status for clarity and consistency with type
                    if (stockQuantity <= 0) {
    ...
    const addEquipment = (equipment, quantity, selectedDate) => {
        if (!selectedDate) {
            toast({
        ...
        setBookingData(prev => {
            ...
            if (existingItemInCart) {
                ...
                return {
                    ...prev,
                    items: prev.items.map(item => item.equipment_id === equipment.id
                        ? {
                            ...item,
    ...
    const submitBooking = async (e) => {
        ...
        try {
            ...
            const bookingItemsPayload = bookingData.items.map(item => ({
