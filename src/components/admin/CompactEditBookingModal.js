...
export const CompactEditBookingModal = ({ booking, onBookingUpdated, onClose, open }) => {
    ...
    const { selectedEquipment, bookingItems, setSelectedEquipment, setBookingItems, addEquipment, updateQuantity, removeItem } = useBookingEquipment();
    useEffect(() => {
        if (booking && open) {
    ...
    const handleUpdateQuantity = (equipmentId, change) => {
    ...
    return (_jsx(Sheet, { open: open, onOpenChange: onClose, children: _jsxs(SheetContent, { className: "w-[500px] sm:w-[540px] overflow-hidden", children: [_jsx(SheetHeader, { children: _jsxs(SheetTitle, { children: [booking.status === 'undeliverable' ? 'Reschedule Delivery' : 'Edit Booking', " #", booking.id.substring(0, 8)] }) }), _jsxs("div", { className: "h-full flex flex-col py-4", children: [_jsxs("div", { className: "flex-1 space-y-4 overflow-y-auto pr-2", children: [_jsx(CustomerInfoSection, { customerInfo: customerInfo, onCustomerInfoChange: setCustomerInfo }), _jsx(DateSelectionSection, { startDate: startDate, endDate: endDate, onStartDateChange: setStartDate, onEndDateChange: setEndDate }), _jsx(EquipmentSelectionSection, { selectedEquipment: selectedEquipment, bookingItems: bookingItems, onSelectedEquipmentChange: setSelectedEquipment, onAddEquipment: handleAddEquipment, onUpdateQuantity: handleUpdateQuantity, onRemoveItem: removeItem })] }), _jsx("div", { className: "border-t pt-4", children: _jsx(BookingSummarySection, { startDate: startDate, endDate: endDate, bookingItems: bookingItems, discount: discount, loading: loading, isUndeliverable: booking.status === 'undeliverable', onDiscountChange: setDiscount, onSubmit: handleSubmit, onCancel: onClose }) })] })] }) }));
};
