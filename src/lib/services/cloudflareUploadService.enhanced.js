...
class EnhancedCloudflareUploadService {
     ...
     */
    async uploadImage(file, options = {}, onProgress) {
     ...
     */
    async uploadWithRetry(file, options, onProgress, attempt = 1) {
        try {
            ...
            try {
                ...
                if (error) {
                    throw new AppError(`Upload failed: ${error.message}`, 'UPLOAD_ERROR', 500);
                }
                if (!data) {
                    throw new AppError('No response data received', 'NO_DATA_ERROR', 500);
                }
     ...
     */
    async uploadMultipleImages(files, options = {}, onProgress) {
        logger.info('Starting batch upload', { fileCount: files.length });
        const results = [];
        const errors = [];
        for (let i = 0; i < files.length; i++) {
            try {
                const result = await this.uploadImage(files[i], options, (progress) => onProgress?.(i, progress));
                results.push(result);
            }
            catch (error) {
     ...
     */
    async preloadImage(imageId, variant = 'public') {
