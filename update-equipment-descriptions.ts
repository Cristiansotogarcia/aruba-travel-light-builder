...
async function updateEquipmentDescriptions() {
  ...
  for (const update of equipmentUpdates) {
    ...
    // Combine description with usage info if present
    let fullDescription = update.newDescription;
    if (update.usageInfo && update.usageInfo.length > 0) {
      if (Array.isArray(update.usageInfo)) {
        fullDescription += "<br><br><strong>Important Usage Information:</strong><br>" + 
                          update.usageInfo.join("<br>");
      } else {
        fullDescription += "<br><br><strong>Important Usage Information:</strong><br>" + 
                          update.usageInfo;
      }
  ...
  console.log('Equipment description updates completed.');
}

updateEquipmentDescriptions().catch(console.error);
